...
build_uncertain_rhs_model(Env& t_env, const Problems::FLP::Instance& t_instance) {
    ...
    for (auto i : Range(n_facilities)) {
        model.add_ctr(
            idol_Sum(j, Range(n_customers), t_instance.demand(j) * y[i][j]) <= t_instance.capacity(i),
...
build_uncertain_technology_matrix_model(Env& t_env, const Problems::FLP::Instance& t_instance) {
    ...
    for (auto i : Range(n_facilities)) {
        const auto c = model.add_ctr(
            idol_Sum(j, Range(n_customers), t_instance.demand(j) * y[i][j]) <= t_instance.capacity(i) * x[i],
            "capacity_" + std::to_string(i)
        );
        robust_description.set_uncertain_mat_coeff(c, x[i], t_instance.capacity(i) * u[i]);
    }
